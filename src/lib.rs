use chrono::{DateTime, Datelike, FixedOffset, NaiveDate, NaiveDateTime, TimeZone, Timelike, Utc, Duration, Month};
use std::ops::{Add, Sub};
        ...
        T: TimeZone,
{
    ...
    fn beginning_of_minute(&self) -> DateTime<Self::Timezone> {
        let local_date_time = Utc::now().with_timezone(self).naive_local();
        let time5 = NaiveDate::from_ymd(local_date_time.year(), local_date_time.month(), local_date_time.day())
    ...
    fn beginning_of_week(&self) -> DateTime<Self::Timezone> {
        let local_date_time = Utc::now().with_timezone(self);
        let prec_day = local_date_time.weekday().number_from_monday() - 1;
        let time: DateTime<T> = local_date_time.sub(Duration::days(prec_day as i64));
        let succ_local_date_time = time.naive_local();
        let time5 = NaiveDate::from_ymd(succ_local_date_time.year(), succ_local_date_time.month(), succ_local_date_time.day())
            .and_hms(0, 0, 0);
        self.from_local_datetime(&time5).unwrap()
    }
    ...
    fn beginning_of_quarter(&self) -> DateTime<Self::Timezone> {
        ...
        let time5 = NaiveDate::from_ymd(local_date_time.year(), month, 1)
            .and_hms(0, 0, 0);
        self.from_local_datetime(&time5).unwrap()
    }
    ...
    fn beginning_of_year(&self) -> DateTime<Self::Timezone> {
        let local_date_time = Utc::now().with_timezone(self).naive_local();
        let time5 = NaiveDate::from_ymd(local_date_time.year(), 1, 1)
            .and_hms(0, 0, 0);
        self.from_local_datetime(&time5).unwrap()
    }
    ...
    fn end_of_minute(&self) -> DateTime<Self::Timezone> {
        let local_date_time = Utc::now().with_timezone(self).naive_local();
        let time5 = NaiveDate::from_ymd(local_date_time.year(), local_date_time.month(), local_date_time.day())
            .and_hms(local_date_time.hour(), local_date_time.minute(), 59);
        self.from_local_datetime(&time5).unwrap()
    }
    ...
    fn end_of_hour(&self) -> DateTime<Self::Timezone> {
        let local_date_time = Utc::now().with_timezone(self).naive_local();
        let time5 = NaiveDate::from_ymd(local_date_time.year(), local_date_time.month(), local_date_time.day())
            .and_hms(local_date_time.hour(), 59, 59);
        self.from_local_datetime(&time5).unwrap()
    }

    fn end_of_day(&self) -> DateTime<Self::Timezone> {
        let local_date_time = Utc::now().with_timezone(self).naive_local();
        let time5 = NaiveDate::from_ymd(local_date_time.year(), local_date_time.month(), local_date_time.day()).and_hms(23, 59, 59);
        self.from_local_datetime(&time5).unwrap()
    }

    fn end_of_week(&self) -> DateTime<Self::Timezone> {
        let local_date_time = Utc::now().with_timezone(self);
        let succ_day = 7 - local_date_time.weekday().number_from_monday();
        let time: DateTime<T> = local_date_time.add(Duration::days(succ_day as i64));
        let succ_local_date_time = time.naive_local();
        let time5 = NaiveDate::from_ymd(succ_local_date_time.year(), succ_local_date_time.month(), succ_local_date_time.day())
            .and_hms(23, 59, 59);
        self.from_local_datetime(&time5).unwrap()
    ...
    fn end_of_month(&self) -> DateTime<Self::Timezone> {
        ...
        let time5 = NaiveDate::from_ymd(year, month, 1)
            .and_hms(0, 0, 0);
        self.from_local_datetime(&time5).unwrap().sub(Duration::seconds(1))
    }
    ...
    fn end_of_year(&self) -> DateTime<Self::Timezone> {
        let local_date_time = Utc::now().with_timezone(self).naive_local();
        let time5 = NaiveDate::from_ymd(local_date_time.year(), 12, 31)
            .and_hms(23, 59, 59);
        self.from_local_datetime(&time5).unwrap()
    }
}
...
#[cfg(test)]
mod test {
    use chrono::{Timelike, FixedOffset};
    use crate::Now;
